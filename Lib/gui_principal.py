...
class SimuladorGUI:
    ...
    def _load_sim_from_disk(self, run_dir, cfg_loaded: dict):
        ...
        try:
            ...
            # Reconstruir mapa pid → color usando conteos del config guardado,
            # igual que construir_particulas() asigna IDs secuenciales por especie.
            _colores_por_id = {}
            _conteos_cfg = cfg_loaded.get("conteos", {})
            _gid = 0
            for _nombre_esp, _n_esp in _conteos_cfg.items():
                _color_esp = ESP_CFG.get(_nombre_esp, {}).get("color", T["cyan"])
                for _j in range(_n_esp):
                    _colores_por_id[_gid] = _color_esp
                    _gid += 1
            ...
            if traj_dir.exists():
                ...
                for csv_path in csvs:
                    try:
                        ...
                        particulas.append(p)
                        # Usar el color de la especie correspondiente al pid;
                        # si el pid no está en el mapa (config antiguo sin conteos),
                        # caer a cyan como antes.
                        colores.append(_colores_por_id.get(pid, T["cyan"]))
                    except Exception:
                        pass
